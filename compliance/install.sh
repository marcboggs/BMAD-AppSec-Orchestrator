#!/usr/bin/env bash
set -euo pipefail
AGENT_NAME="compliance"; AGENT_LABEL="Compliance"; AGENT_DESC="Regulatory framework mapper"
...
# Dependency 2: threat-model
if [ -f "$KIRO_AGENTS_DIR/threat-model.json" ]; then ok "threat-model is installed"
else
    warn "threat-model is required but not installed"
    INSTALLER="$(dirname "$SCRIPT_DIR")/threat-model/install.sh"
    if [ -f "$INSTALLER" ]; then echo -e "  ${CYAN}→ Installing threat-model...${RESET}"; bash "$INSTALLER"
    else fail "Cannot find threat-model installer. Install it first."; exit 1; fi
fi
echo ""
