<#
.SYNOPSIS
    Installs the api-spec-review Kiro agent.
.DESCRIPTION
    REQUIRES: secreview (for SCA correlation on API dependencies).
#>
$ErrorActionPreference = "Stop"
$AgentName = "api-spec-review"; $AgentLabel = "API Spec Review"; $AgentDesc = "OWASP API Top 10 reviewer"
...
$dep = Join-Path $KiroAgentsDir "secreview.json"
if (Test-Path $dep) { Write-Ok "secreview is installed" }
else {
    Write-Warn "secreview is required but not installed"
    Write-Info "API Spec Review uses secreview for SCA correlation"
    Write-Host ""
    $installer = Join-Path (Join-Path (Split-Path $ScriptDir) "secreview") "install.ps1"
    if (Test-Path $installer) { Write-Host "  → Installing secreview..." -ForegroundColor Cyan; & $installer }
