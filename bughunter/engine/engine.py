...
State is persisted after every step, so a run is auditable and resumable.
  ...
  python3 engine/engine.py --scope my-engagement.json --max-hunts 8             # live (needs Burp + kiro-cli/claude auth + budget)
...
def main():
    ...
    ap.add_argument("--base", default="~/.kiro/bughunter/engagements")
