...
Storage (~/.kiro/bughunter/memory/, override BUGHUNTER_MEMORY_DIR):
...
# fcntl is Unix-only. On Windows (a supported Kiro host) it doesn't exist, and
# importing it at top level would crash `import memory` — which engine.py does
# unconditionally. Guard it and degrade to a best-effort lock so the engine still
# imports and runs cross-platform. (The append+rotate critical section is only
# contended under --parallel with the same ledger dir.)
try:
    import fcntl  # type: ignore
    _HAVE_FCNTL = True
except ImportError:  # Windows / Kiro-on-Windows
...
def _root():
    return os.path.expanduser(os.environ.get("BUGHUNTER_MEMORY_DIR", "~/.kiro/bughunter/memory"))
...
def _append(path, obj):
    ...
    if not _HAVE_FCNTL:
        # Windows / no fcntl: best-effort append. Rotate then append; the
        # cross-process lock isn't available, but single-process (incl. the
        # engine's ThreadPool, which is GIL-serialized on these short writes)
        # is safe enough for the ledger's use.
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dir", default=None, help="ledger dir (default ~/.kiro/bughunter/memory)")
