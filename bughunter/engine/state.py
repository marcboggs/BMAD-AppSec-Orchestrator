...
class Engagement:
    ...
    def log(self, msg):
        ...
        with open(self.log_path, "a", encoding="utf-8") as f:
            f.write(line + "\n")
        # Windows consoles are often cp1252; the log text uses unicode glyphs
        # (→ ⚠ ✓). Never let an encoding error crash a phase — fall back to an
        # ASCII-safe render for stdout only (the file always keeps full UTF-8).
        try:
