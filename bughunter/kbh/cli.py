...
REPO_ROOT = Path(__file__).resolve().parent.parent   # the bughunter/ agent dir
SKILLS_DIR = REPO_ROOT / "skills"
REPORTS_DIR = REPO_ROOT / "disclosed-reports"

# Clone mode = the live agent tree is present (skills/ on disk). Otherwise we're
# pip-installed and fall back to the bundled kbh/data/skill_index.json.
...
def _bundled_index() -> dict:
    ...
    if _BUNDLED_INDEX is None:
        try:
            ...
            raw = (files("kbh") / "data" / "skill_index.json").read_text(encoding="utf-8")
        except Exception:
...
def say(s: str = ""):
    try:
        ...
        # Windows cp1252 console + unicode glyphs (→ ⚠ ✓ 📖): degrade gracefully
...
def has_cmd(name: str) -> bool:
    # shutil.which is cross-platform (honors PATHEXT on Windows), unlike the
    # Unix-only `which` binary the upstream used.
    import shutil
...
def configure_http_proxy(proxy_url: str | None = None, insecure: bool = False) -> tuple[bool, str]:
    ...
    Resolution order when proxy_url is not given:
      1. KBH_BURP_PROXY (or legacy CBH_BURP_PROXY) env var (the tool's own Burp var → implies insecure)
      2. HTTPS_PROXY / HTTP_PROXY env vars (ambient/corporate → TLS stays verified)
    """
    global _HTTP_OPENER
    if not proxy_url:
        burp_env = os.environ.get("KBH_BURP_PROXY") or os.environ.get("CBH_BURP_PROXY")
        if burp_env:
...
def cmd_recon(args: argparse.Namespace) -> int:
    ...
    say()
    say(f"  Next: {color('kbh classify <url>', 'bold')} for fast pattern-match, or {color('hunt <target>', 'bold')} in the bughunter agent (kiro-cli --agent bughunter, or /hunt in Claude Code) for full LLM-driven hunting")
    return 0
...
def write_recon_summary(target: str, subs: set[str], resolved: dict, live: list, out: Path):
    lines = [f"# Recon — {target}",
             "",
             f"_Generated by `kbh recon {target}` at {datetime.datetime.now().isoformat(timespec='seconds')}._",
             "",
    ...
    lines += [
        ...
        "",
        "- For each live host, run `kbh classify https://<host>/<path>?<params>` to surface attack candidates.",
...
def build_manifest(target: str, subs: set, resolved: dict, live: list) -> dict:
    """Assemble the recon→hunt manifest. kbh fills assets + ranked_surface; the
...
def cmd_classify(args: argparse.Namespace) -> int:
    ...
    if not result["matches"]:
        ...
        say(f"    {color('kbh recon <target>', 'bold')} to map attack surface first.")
        return 1
    say()
    ...
    say(f"  4. {color('kbh triage <finding.md>', 'bold')} once you have a candidate finding")
    say()
    say(color("Need richer context? In the bughunter agent (kiro-cli --agent bughunter, or Claude Code):", "dim"))
    say(color(f"   hunt <url>      — full hunt-dispatch routing with LLM judgment (/hunt in Claude Code)", "dim"))
    say(color(f"   chain           — build A→B→C exploit chains (/chain in Claude Code)", "dim"))
    return 0
...
def cmd_triage(args: argparse.Namespace) -> int:
    ...
    if not fail_qs:
        ...
        say(color("Eligible for report drafting via `kbh report <finding.md>`.", "dim"))
        return 0
...
def cmd_surface(args: argparse.Namespace) -> int:
    ...
    if not mpath or not mpath.exists():
        print(f"[error] no recon manifest for {target}. Run:  kbh recon {target}", file=sys.stderr)
        return 1
    try:
    ...
    say()
    say(f"  Next: {color('hunt ' + target, 'bold')} in the bughunter agent (/hunt in Claude Code), "
        f"or {color('kbh classify <url>', 'bold')} for a single URL")
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="kbh",
        description=("kiro-bughunter CLI — terminal-native deterministic runner.\n"
                     "Terminal-native deterministic runner for the Kiro bughunter agent. "
                     "Use kbh for CI/CD, scripted runs, and deterministic verification; use "
                     "the bughunter agent's workflow commands (hunt/recon/triage/report) for "
                     "LLM-driven hunting."),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            Examples:
              kbh recon hackerone.com
              kbh recon target.com --burp                  # route via Burp proxy
              kbh classify "https://api.target.com/v1/users/42?next=https://evil.com"
              kbh triage findings/idor-2026-05-15.md
              kbh report findings/idor-2026-05-15.md --platform bugcrowd --out draft.md

            For LLM-driven hunting with full skill context, use the bughunter agent's
            workflow commands: hunt / recon / triage / report / validate / chain / autopilot
            (kiro-cli --agent bughunter; the same commands are /hunt … slash commands in Claude Code).
            See docs/kbh-cli.md for the "when to use which" matrix.
            """),
    )
